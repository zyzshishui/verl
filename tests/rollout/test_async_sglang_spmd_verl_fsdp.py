...
from torch.distributed.fsdp import FullyShardedDataParallel as FSDP, MixedPrecision, ShardingStrategy, StateDictType
...
def test_async_sglang_spmd_verl_fsdp():
    ...
    actor_model = AutoModelForCausalLM.from_pretrained(local_model_path, torch_dtype=torch.bfloat16, device_map="cuda")
    ...
    print("call release_memory_occupation")
    ...
    FSDP.set_state_dict_type(fsdp_model, state_dict_type=StateDictType.FULL_STATE_DICT)
    ...
    print("done updating rollout weights", flush=True)
    idx_list = []
